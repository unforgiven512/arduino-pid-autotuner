...
void setTuningParams(pid_autotune_params_t pidParams) {
	targetInputValue = pidParams->pid_autotune_target;
	loopInterval = pidParams->pid_autotune_interval;
	minOutput = pidParams->pid_autotune_outrange_min;
	maxOutput = pidParams->pid_autotune_outrange_max;
	znMode = pidParams->pid_autotune_zn;
	cycles = pidParams->pid_autotune_tune_cycles;
}
...
void getPIDParams(pid_autotune_results_t *pidResults) {
	pidResults->pid_autotune_kp = kp;
	pidResults->pid_autotune_ki = ki;
	pidResults->pid_autotune_kd = kd;
}
